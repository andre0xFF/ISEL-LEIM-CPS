...
def constellation_encode(x: np.ndarray) -> (np.ndarray, np.int):
    ...
    for i in range(0, len(coords[0]), 2):
        signal = -2 * x[:, i * 2:i * 2 + 2] + 1
        coord = 2 * x[:, i * 2 + 2:i * 2 + 4] + 1

        coords[:, i:i + 2] = signal * coord
...
def qam_decode(y: np.ndarray, p: np.int, rm_bits: np.int) -> np.ndarray:
    ...
    for i in range(0, len(coords[0]), 2):
        j = np.int(p / 2 * i)
        phi_x = np.sum(y[:, j:j + p] * c1, axis=1)
        phi_y = np.sum(y[:, j:j + p] * c2, axis=1)

        phi_x = np.round(2 * np.ceil(phi_x / 2)) - 1
        phi_y = np.round(2 * np.ceil(phi_y / 2)) - 1
